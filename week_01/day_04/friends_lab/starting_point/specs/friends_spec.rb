...
class TestFriends < MiniTest::Test
  ...
  def test_get_favourite_tv_show
    result = get_favourite(:tv_show, @person5)
  ...
  def test_add_new_friend
    ...
    num_of_friends = @person2[:friends].length()
    assert_equal(2, num_of_friends)
  end
  ...
  def test_all_snacks_concat
    result = all_items(@people, :snacks)
    assert_equal(
      ['charcuterie', 'soup', 'bread', 'Scooby snacks', 'spaghetti', 'ratatouille', 'spinach'], result)
  end

  # 9. Find people with no friends
  # (hint: return an array, there might be more people in the future with no friends!)
  def test_nae_pals
    result = get_loners(@people)

...
- Use RESTful routes to GET POST PUT and DELETE those objects from the database, to create a RESTful API with additional custom routes
...
- **Spring Framework** to help create controllers and **Spring Data REST** for Building RESTful services, so that we do not need to define the restful routes, or the conventional behaviour for those restful routes

We will get all of the above functionality that out of the box.
...
This points spring to a database `pireateservice` running on localhost, sets the dialect to understand Postgres Database types and allows us to use "Hibernate" which is a library for helping us manage database access.
...
> If the `postgres` user doesn't exist one can be created with `createuser postgres`

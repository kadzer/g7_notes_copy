...
- So we know which operators we can use. As demonstrated in the following example, can use the addition operator (`+`) when dealing with string types, but not the subtraction operator (`-`), which gives us `NaN` (Not-A-Number). More on that later.
...
1. falsy values:
	...
	- null
	- undefined
2. `!0` evaluates as true
3. `!0 && !1` evaluates as false
4. Because strict equals checks that both the type and value match, and avoids unexpected behaviour due to type coercion.
